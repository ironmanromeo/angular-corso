...
export function ComponenteDecoratore (patametri:ParametriDecoratore){
    return function (costruttore:Function){
        const ref = document.getElementById(patametri.selettore)
        if (ref){
            ref.innerHTML = patametri.template
        }
    }
}